...
def extract(request: Request, payload: ExtractRequest, current_user: User = Depends(get_current_user)):
    # current_user isn't used in the body -- extract() is a stateless AI
    # call with no DB lookup -- but it still requires login and now a
    # rate limit, so an anonymous caller can't burn Gemini quota with
    # zero accountability or ceiling.
...
def save(payload: SaveRequest, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    # No rate limit needed -- this just persists already-extracted,
    # already-reviewed data. No Gemini call happens here.
